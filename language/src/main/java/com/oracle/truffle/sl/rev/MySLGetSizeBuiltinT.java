...
public class MySLGetSizeBuiltinT extends MySLBuiltinNodeT implements SLGetSizeBuiltinT {
    private final SLGetSizeBuiltin it;
    private final SLExpressionNodeT arguments0_;
    @CompilerDirectives.CompilationFinal
    private int state_;


    public MySLGetSizeBuiltinT(ExecSLRevisitor execSLRevisitor, SLGetSizeBuiltin it) {
        super(it);
        this.it = it;
    ...
    public Object execute(VirtualFrame frameValue) {
        ...
        if (state != 0 /* is-active getSize(TruffleObject) */ && arguments0Value_ instanceof TruffleObject) {
            TruffleObject arguments0Value__ = (TruffleObject) arguments0Value_;
            return getSize(arguments0Value__);
        }
    ...
    private Object executeAndSpecialize(Object arguments0Value) {
        ...
        try {
            if (arguments0Value instanceof TruffleObject) {
                ...
                this.state_ = state = state | 0b1 /* add-active getSize(TruffleObject) */;
                lock.unlock();
                hasLock = false;
                return getSize(arguments0Value_);
            }
            CompilerDirectives.transferToInterpreterAndInvalidate();
    ...
    public Object getSize(TruffleObject obj) {
        try {
            return ForeignAccess.sendGetSize(it.getGetSize(), obj);
        } catch (UnsupportedMessageException e) {
            throw new SLException(e.getMessage(), it);
        }
    }
}
