...
public class MySLMulNodeT implements SLMulNodeT {
    ...
    public Object executeGeneric(VirtualFrame frameValue) {
        int state = state_;
        if ((state & 0b110) == 0 /* only-active mul(long, long) */ && (state & 0b111) != 0  /* is-not mul(long, long) && mul(SLBigNumber, SLBigNumber) && typeError(Object, Object) */) {
    ...
    private Object executeGeneric_long_long0(VirtualFrame frameValue, int state) {
        ...
        assert (state & 0b1) != 0 /* is-active mul(long, long) */;
        try {
            ...
            try {
                ...
                this.state_ = this.state_ & 0xfffffffe /* remove-active mul(long, long) */;
            } finally {
                lock.unlock();
            }
    ...
    private Object executeGeneric_generic1(VirtualFrame frameValue, int state) {
        ...
        if ((state & 0b10) != 0 /* is-active mul(SLBigNumber, SLBigNumber) */ && SLTypesGen.isImplicitSLBigNumber((state & 0b11000) >>> 3 /* extract-implicit-active 0:SLBigNumber */, leftNodeValue_)) {
    ...
    public long executeLong(VirtualFrame frameValue) throws UnexpectedResultException {
        ...
        if ((state & 0b1) != 0 /* is-active mul(long, long) */) {
            try {
                return mul(leftNodeValue_, rightNodeValue_);
            } catch (ArithmeticException ex) {
                // implicit transferToInterpreterAndInvalidate()
    ...
    private Object executeAndSpecialize(Object leftNodeValue, Object rightNodeValue) {
        ...
        try {
            if ((exclude) == 0 /* is-not-excluded mul(long, long) */ && leftNodeValue instanceof Long) {
                long leftNodeValue_ = (long) leftNodeValue;
                if (rightNodeValue instanceof Long) {
                    long rightNodeValue_ = (long) rightNodeValue;
                    this.state_ = state = state | 0b1 /* add-active mul(long, long) */;
                    try {
                        lock.unlock();
                        hasLock = false;
                        return mul(leftNodeValue_, rightNodeValue_);
            ...
            }
            {
                ...
                if ((sLBigNumberCast0 = SLTypesGen.specializeImplicitSLBigNumber(leftNodeValue)) != 0) {
                    ...
                    if ((sLBigNumberCast1 = SLTypesGen.specializeImplicitSLBigNumber(rightNodeValue)) != 0) {
                        ...
                        this.state_ = state = state | 0b10 /* add-active mul(SLBigNumber, SLBigNumber) */;
                        lock.unlock();
                        hasLock = false;
                        return mul(leftNodeValue_, rightNodeValue_);
                    }
